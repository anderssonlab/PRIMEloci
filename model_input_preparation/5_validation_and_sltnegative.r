...
writeLines("\n..")
tr_pos_sltocr_grl <- select_ocr_on_condition(tr_orig_ctss,
                                             ocr_train_gr)
tr_neg_sltocr_grl <- select_ocr_on_condition(tr_orig_ctss,
                                             ocrlike_neg_train_gr)

tr_pos_aug_sltocr_grl <- select_ocr_on_condition(tr_orig_ctss,
                                                 ocr_train_aug_gr)
tr_neg_aug_sltocr_grl <- select_ocr_on_condition(tr_orig_ctss,
                                                 ocrlike_neg_train_aug_gr)

te_pos_sltocr_grl <- select_ocr_on_condition(te_orig_ctss,
                                             ocr_test_gr)
te_neg_sltocr_grl <- select_ocr_on_condition(te_orig_ctss,
                                             ocrlike_neg_test_gr)
...
tr_neg_grl <- select_negative_to_match_positive(tr_neg_sltocr_grl,
                                                tr_pos_grl)
tr_neg_aug_grl <- select_negative_to_match_positive(tr_neg_aug_sltocr_grl,
                                                    tr_pos_aug_grl)
te_neg_grl <- select_negative_to_match_positive(te_neg_sltocr_grl,
                                                te_pos_grl)


# TRAINING: split training (80%) and validation (20%)
# based on their natural ratio of annotation
tr_vl_ls <- wrapup_split_train_valid(tr_pos_grl, tr_neg_grl)
...
for (i in seq_along(te_pos_grl)) {
  final_num_te <- min(length(te_pos_grl[[i]]), length(te_neg_grl[[i]]))

  pos_te_grl[[i]] <- keep_equal_num(te_pos_grl[[i]], final_num_te)
  neg_te_grl[[i]] <- keep_equal_num(te_neg_grl[[i]], final_num_te)
}
...
writeLines("\nSaving..")
save(
  list = c("tr_vl_ls", "tr_vl_aug_ls", "te_ls"),
